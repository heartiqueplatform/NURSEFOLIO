        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={`flex items-start text-left p-4 rounded-xl border transition-all duration-200 active:scale-[99%] cursor-pointer ${
                isSelected 
                  ? 'border-teal-600 bg-teal-50/25 ring-2 ring-teal-600/10' 
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span className="block font-semibold text-slate-800 text-sm">{opt.label}</span>
                <span className="block text-xs text-slate-500 mt-1 line-clamp-2 leading-relaxed">{opt.desc}</span>
              </div>
            </button>
          );
        })}
