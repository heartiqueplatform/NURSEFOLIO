  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Focus Designations & Board Licenses</h2>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border-2 border-teal-100 p-6 shadow-sm animate-in slide-in-from-top duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs text-slate-705 font-medium">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-slate-500 mb-1">Certification Name</label>
                <input
                  required
                  type="text"
                  value={name}
              ...
              <div>
                <label className="block text-slate-500 mb-1">Issuing Organization</label>
                <input
                  required
                  type="text"
                  value={issuingOrg}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-slate-500 mb-1">Verification URL (Optional)</label>
                <input
                  type="url"
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center text-slate-500 shadow-sm">
          ...
          <h4 className="font-bold text-slate-800">No designations listed</h4>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {certs.map((cert) => (
            <div key={cert.id} className="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm hover:shadow-md transition">
              <div className="flex justify-between items-start mb-4">
                <div className="w-10 h-10 rounded-xl bg-teal-50 text-teal-600 flex items-center justify-center">
                  <Award className="w-6 h-6" />
                </div>
                <div className="flex gap-1">
                  <button onClick={() => handleEditClick(cert)} className="text-slate-400 hover:text-teal-600 p-1.5 rounded-lg hover:bg-teal-50"><Pencil className="w-4 h-4" /></button>
                  <button onClick={() => setDeleteId(cert.id)} className="text-slate-400 hover:text-rose-600 p-1.5 rounded-lg hover:bg-rose-50"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
              <h4 className="font-extrabold text-slate-900 text-sm">{cert.name}</h4>
              <p className="text-[11px] font-bold text-teal-700">{cert.issuing_organization}</p>
              <div className="mt-3 pt-3 border-t border-slate-50 text-[10px] text-slate-500 flex justify-between">
                <span>Issued: <span className="font-semibold text-slate-700">{cert.issue_date}</span></span>
                {cert.verification_url && (
                  <a href={cert.verification_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
