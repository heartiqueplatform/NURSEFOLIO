  ...
  return (
    <div className="space-y-6 font-sans text-xs">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-4">
        <div>
          <h3 className="text-base font-extrabold text-slate-900 tracking-tight">Active Availability Status</h3>
          <p className="text-slate-500 text-xs mt-0.5 font-medium">Set your active placement indications for hospitals & medical agencies.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <button
            ...
            className={`flex items-center gap-3 p-4 border rounded-xl select-none transition-all active:scale-98 cursor-pointer ${
          ...
          >
            ...
            <div className="text-left">
              ...
              <span className="block text-[9px] font-normal text-slate-500 mt-0.5">Available for round hire</span>
            </div>
          </button>
          ...
          <button
            ...
            className={`flex items-center gap-3 p-4 border rounded-xl select-none transition-all active:scale-98 cursor-pointer ${
              availability === 'open'
                ? 'border-amber-500 bg-amber-50/20 text-amber-850 font-bold'
                : 'border-slate-150 bg-white hover:bg-slate-50 text-slate-700'
            }`}
          >
            ...
            <div className="text-left">
              ...
              <span className="block text-[9px] font-normal text-slate-500 mt-0.5">Considering placement options</span>
            </div>
          </button>
          ...
          <button
            ...
            className={`flex items-center gap-3 p-4 border rounded-xl select-none transition-all active:scale-98 cursor-pointer ${
              availability === 'busy'
                ? 'border-slate-500 bg-slate-50/30 text-slate-710 font-bold'
                : 'border-slate-150 bg-white hover:bg-slate-50 text-slate-700'
            }`}
          >
            ...
            <div className="text-left">
              ...
              <span className="block text-[9px] font-normal text-slate-500 mt-0.5">Currently fully placed</span>
            </div>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-4">
        <div className="border-b border-slate-100 pb-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <h3 className="text-base font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
              <ShieldCheck className="w-5.5 h-5.5 text-teal-605" />
        ...
        {verSuccess && (
          <div className="bg-emerald-50 border border-emerald-100 text-emerald-705 p-3.5 rounded-xl text-xs font-semibold flex items-center gap-2">
        ...
        {user.verification_status === 'verified' ? (
          <div className="p-4 bg-emerald-50/40 border border-emerald-110 text-emerald-805 rounded-xl">
            <p className="font-bold flex items-center gap-1">
              <ShieldCheck className="w-4 h-4 text-emerald-600 animate-bounce" />
              <span>Full Board Verification Verified!</span>
            </p>
            <p className="mt-1 text-slate-500 font-normal leading-relaxed text-[11px]">
              Your nursing license matches state regulators. No further verification check is required at this time.
            </p>
          </div>
        ) : user.verification_status === 'pending' ? (
          <div className="p-4 bg-amber-50/40 border border-amber-100 text-amber-805 rounded-xl">
            <p className="font-bold flex items-center gap-1">
          ...
          <form onSubmit={handleVerificationSubmit} className="space-y-4 text-xs font-medium text-slate-700">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-slate-500 mb-1">Board Designations Type</label>
                <select
                  id="ver-license-type"
                  value={licenseType}
                  onChange={(e) => setLicenseType(e.target.value)}
                  className="w-full px-3 py-2 bg-slate-50 rounded-xl border border-slate-205 focus:outline-none focus:border-teal-400 focus:bg-white text-slate-705"
                >
              ...
              <div>
                <label className="block text-slate-500 mb-1">License ID / Student Card serial</label>
                <input
                  id="ver-license-num"
                  required
                  type="text"
                  ...
                  className="w-full pl-3 pr-4 py-2 bg-slate-50 rounded-xl border border-slate-205 focus:outline-none focus:border-teal-400 focus:bg-white text-slate-805"
                />
              </div>

              <div>
                <label className="block text-slate-500 mb-1">State / Country regulatory Board</label>
                <input
            ...
            <div>
              <button
                ...
                className="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2.5 px-6 rounded-xl cursor-pointer active:scale-95 transition flex items-center justify-center gap-2"
              >
                {submittingVer ? (
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-4">
        <div>
          <h3 className="text-base font-extrabold text-slate-900 tracking-tight flex items-center gap-1.5">
            <Key className="w-5.5 h-5.5 text-teal-605" />
            <span>Update Security coordinates</span>
          </h3>
          <p className="text-slate-550 text-xs font-normal">Manage email registrations details and password changes.</p>
        </div>

        {passMsg && (
          <div className="bg-emerald-50 border border-emerald-100 text-emerald-705 p-3.5 rounded-xl text-xs font-semibold">
            {passMsg}
          </div>
        )}

        <form onSubmit={handlePasswordSubmit} className="space-y-4 text-xs font-semibold text-slate-700 max-w-md">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-slate-500 mb-1">Create New Password</label>
              <input
                id="sec-new-pass"
                type="password"
                value={newPassword}
                ...
                placeholder="••••••••"
                className="w-full pl-3 pr-4 py-2 bg-slate-50 rounded-xl border border-slate-205 focus:outline-none focus:border-teal-400 focus:bg-white text-slate-855"
              />
            </div>
          </div>
          ...
          <button
            id="save-sec-btn"
            type="submit"
            className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl active:scale-95 transition text-[11px] font-bold"
          >
            Update password
          </button>
        </form>
