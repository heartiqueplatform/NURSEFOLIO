...
export default function LegalPages() {
  ...
  return (
    <div className="bg-slate-50 dark:bg-zinc-950 min-h-screen py-6 mt-16 md:py-10">
      <div className="max-w-5xl mx-auto px-3 md:px-4 lg:px-8">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center mb-6 md:mb-8">
          <div>
            <nav className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-xs text-slate-500 dark:text-slate-400 font-semibold mb-1.5 md:mb-2">
            ...
            <p className="text-[10px] md:text-sm text-slate-500 dark:text-slate-400 mt-0.5 md:mt-1">
          ...
          <button
            id="btn-print-legal"
            onClick={handlePrint}
            className="w-full md:w-auto flex items-center justify-center gap-1.5 md:gap-2 px-4 py-2 md:py-2 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 bg-white dark:bg-zinc-950 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold text-[10px] md:text-xs rounded-lg md:rounded-xl md:shadow-sm cursor-pointer transition-all duration-150"
          >
        ...
        {/* Tab System Wrapper */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-0 md:gap-8">

          {/* Navigation panel - horizontal scroll on mobile */}
          <div className="lg:col-span-1 flex flex-row lg:flex-col gap-1.5 md:gap-2 overflow-x-auto pb-3 md:pb-0 -mx-3 md:mx-0 px-3 md:px-0 scrollbar-none">
            <button
              id="tab-privacy"
              onClick={() => handleTabChange('privacy')}
              className={`flex items-center gap-2 md:gap-2.5 px-3 md:px-4 py-2.5 md:py-3 text-left rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold transition-all duration-150 whitespace-nowrap cursor-pointer flex-shrink-0 md:w-full ${activeTab === 'privacy'
                ? 'bg-indigo-600 dark:bg-indigo-700 text-white md:shadow-md md:shadow-indigo-600/10'
                : 'bg-white dark:bg-zinc-950 text-slate-600 dark:text-slate-400 hover:bg-slate-100/70 dark:hover:bg-slate-800 border border-slate-200/50 dark:border-slate-700'
                }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 md:gap-2.5 px-3 md:px-4 py-2.5 md:py-3 text-left rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold transition-all duration-150 whitespace-nowrap cursor-pointer flex-shrink-0 md:w-full ${activeTab === 'terms'
                ? 'bg-indigo-600 dark:bg-indigo-700 text-white md:shadow-md md:shadow-indigo-600/10'
                : 'bg-white dark:bg-zinc-950 text-slate-600 dark:text-slate-400 hover:bg-slate-100/70 dark:hover:bg-slate-800 border border-slate-200/50 dark:border-slate-700'
                }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 md:gap-2.5 px-3 md:px-4 py-2.5 md:py-3 text-left rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold transition-all duration-150 whitespace-nowrap cursor-pointer flex-shrink-0 md:w-full ${activeTab === 'compliance'
                ? 'bg-indigo-600 dark:bg-indigo-700 text-white md:shadow-md md:shadow-indigo-600/10'
                : 'bg-white dark:bg-zinc-950 text-slate-600 dark:text-slate-400 hover:bg-slate-100/70 dark:hover:bg-slate-800 border border-slate-200/50 dark:border-slate-700'
                }`}
            >
              ...
              <span className="hidden md:inline">Compliance & Kenya Regulations</span>
            </button>
          </div>

          {/* Policy Document Content Block - full width on mobile */}
          <div className="lg:col-span-3 bg-white dark:bg-zinc-950 md:border md:border-slate-200/70 md:dark:border-slate-800 md:rounded-2xl md:shadow-sm p-4 md:p-6 lg:p-10 border-t border-slate-200 dark:border-slate-800 md:border-t md:border-slate-200/70">

            {/* TAB 1: PRIVACY POLICY */}
            {activeTab === 'privacy' && (
              <div className="space-y-4 md:space-y-6">
                <div className="border-b border-slate-100 dark:border-slate-800 pb-4 md:pb-5">
                  <div className="flex items-center gap-1.5 md:gap-2 text-indigo-600 dark:text-indigo-400 mb-1 md:mb-1.5 font-bold text-[10px] md:text-xs uppercase tracking-wider">
                    <Shield className="w-3.5 h-3.5 md:w-4 md:h-4" />
                    <span>Kenya DPA 2019 & GDPR Compliant</span>
                  </div>
                  <h2 className="text-lg md:text-xl font-bold font-display text-slate-900 dark:text-white tracking-tight">Privacy Policy</h2>
                  <p className="text-[10px] md:text-xs text-slate-400 dark:text-slate-500 mt-0.5">Last Updated: May 25, 2026. Version 1.2.0</p>
                </div>

                <div className="text-xs md:text-sm text-slate-600 dark:text-slate-400 leading-relaxed space-y-3 md:space-y-4">
                  <p>
                  ...
                  <h3 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white mt-4 md:mt-6 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  <ul className="list-disc pl-4 md:pl-5 space-y-1.5 md:space-y-2 mt-0.5 md:mt-1 text-[11px] md:text-sm">
                  ...
                  <div className="bg-rose-50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-800 p-3 md:p-4 rounded-lg md:rounded-xl text-rose-900 dark:text-rose-300 flex flex-col sm:flex-row gap-2 md:gap-3">
                    <div className="p-1 md:p-1.5 bg-rose-500 text-white rounded-lg h-fit">
                      <UserX className="w-3.5 h-3.5 md:w-4 md:h-4" />
                    </div>
                    <div>
                      <p className="font-bold text-[10px] md:text-xs">Right of Erasure (DPA Sec 26 & GDPR Art 17)</p>
                      <p className="text-[10px] md:text-xs text-rose-900 dark:text-rose-300 mt-0.5 leading-relaxed">
                  ...
                  <ul className="list-disc pl-4 md:pl-5 space-y-1 md:space-y-1.5 mt-0.5 md:mt-1 text-[11px] md:text-sm">
                  ...
                  <div className="pt-3 md:pt-4 border-t border-slate-100 dark:border-slate-800 text-[10px] md:text-xs text-slate-400 dark:text-slate-500 mt-4 md:mt-6 text-center">
            ...
            {activeTab === 'terms' && (
              <div className="space-y-4 md:space-y-6">
                <div className="border-b border-slate-100 dark:border-slate-800 pb-4 md:pb-5">
                  ...
                  <h2 className="text-lg md:text-xl font-bold font-display text-slate-900 dark:text-white tracking-tight">Terms of Service</h2>
                  <p className="text-[10px] md:text-xs text-slate-400 dark:text-slate-500 mt-0.5">Last Updated: May 25, 2026</p>
                </div>
                ...
                <div className="text-xs md:text-sm text-slate-600 dark:text-slate-400 leading-relaxed space-y-3 md:space-y-4">
                  ...
                  <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-800 p-3 md:p-5 rounded-lg md:rounded-xl text-slate-800 dark:text-slate-200">
                    <div className="flex items-start gap-2 md:gap-3">
                      <div className="p-1 outline outline-amber-200 dark:outline-amber-800 outline-offset-1 bg-amber-500 text-white rounded-lg flex-shrink-0">
                        <BadgeAlert className="w-3.5 h-3.5 md:w-4 md:h-4" />
                      </div>
                      <div className="text-[10px] md:text-xs">
                        <p className="font-bold text-slate-900 dark:text-white uppercase tracking-tight text-[10px] md:text-[11px] mb-0.5 md:mb-1">
                        ...
                        <ul className="list-disc pl-3 md:pl-4 space-y-0.5 md:space-y-1 mt-1 md:mt-1.5 text-slate-600 dark:text-slate-400 text-[10px] md:text-xs">
            ...
            {activeTab === 'compliance' && (
              <div className="space-y-4 md:space-y-6">
                <div className="border-b border-slate-100 dark:border-slate-800 pb-4 md:pb-5">
                  ...
                  <h2 className="text-lg md:text-xl font-bold font-display text-slate-900 dark:text-white tracking-tight">Compliance & Regional Standards</h2>
                  <p className="text-[10px] md:text-xs text-slate-400 dark:text-slate-500 mt-0.5">Focus: NCK Portal, Kenya DPA 2019, AU Data Convention</p>
                </div>

                <div className="text-xs md:text-sm text-slate-600 dark:text-slate-400 leading-relaxed space-y-3 md:space-y-4">

                  {/* Kenya Regional Card */}
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-slate-100 dark:border-slate-700 space-y-2.5 md:space-y-3.5">
                    <div className="flex items-center gap-1.5 md:gap-2 text-indigo-700 dark:text-indigo-400">
                      <Map className="w-3.5 h-3.5 md:w-4 md:h-4 flex-shrink-0" />
                    ...
                    <div className="space-y-2 md:space-y-3 text-[11px] md:text-sm">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3.5 pt-1 md:pt-1.5">
                        <div className="bg-white dark:bg-zinc-950 p-2.5 md:p-3.5 rounded-lg md:rounded-xl border border-slate-100 dark:border-slate-700 md:shadow-sm space-y-0.5 md:space-y-1">
                          <div className="flex items-center gap-1 text-teal-600 dark:text-teal-400 text-[10px] md:text-xs font-bold uppercase">
                            <CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5" />
                            <span>ODPC Registration</span>
                          </div>
                          <p className="text-[10px] md:text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">
                      ...
                      <div className="p-2 md:p-3 bg-indigo-50/50 dark:bg-indigo-950/30 rounded-lg md:rounded-xl border border-indigo-100 dark:border-indigo-800 text-[10px] md:text-[11px] text-indigo-900 dark:text-indigo-300 flex gap-1.5 md:gap-2">
                        <FileCheck className="w-3.5 h-3.5 md:w-4 md:h-4 text-indigo-600 dark:text-indigo-400 flex-shrink-0 mt-0.5" />
                        <div>
                  ...
                  {/* Kenya-Specific Regulatory Bodies Section */}
                  <div className="space-y-2 md:space-y-3 mt-3 md:mt-4">
                    <h3 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                      <Building className="w-3.5 h-3.5 md:w-4 md:h-4" />
                      <span>Kenya Regulatory Bodies We Align With</span>
                    </h3>
                    <div className="grid grid-cols-1 gap-2 md:gap-3">
                      <div className="flex items-start gap-2 md:gap-3 p-2.5 md:p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-lg md:rounded-xl">
                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-teal-50 dark:bg-teal-950/50 text-teal-600 dark:text-teal-400 flex items-center justify-center flex-shrink-0">
                          <Shield className="w-3.5 h-3.5 md:w-4 md:h-4" />
                        </div>
                        <div>
                          <p className="text-[10px] md:text-xs font-bold text-slate-800 dark:text-slate-200">Nursing Council of Kenya (NCK)</p>
                          <p className="text-[9px] md:text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Verification of Registered Nurse (RN), Kenya Registered Community Health Nurse (KRCHN), and nursing student licenses.</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2 md:gap-3 p-2.5 md:p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-lg md:rounded-xl">
                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-teal-50 dark:bg-teal-950/50 text-teal-600 dark:text-teal-400 flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                          <p className="text-[10px] md:text-xs font-bold text-slate-800 dark:text-slate-200">Office of the Data Protection Commissioner (ODPC)</p>
                          <p className="text-[9px] md:text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Ensuring compliance with Kenya's Data Protection Act, 2019 for all user data processing activities.</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2 md:gap-3 p-2.5 md:p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-lg md:rounded-xl">
                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-teal-50 dark:bg-teal-950/50 text-teal-600 dark:text-teal-400 flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                          <p className="text-[10px] md:text-xs font-bold text-slate-800 dark:text-slate-200">Kenya Health Professions Oversight Authority (KHPOA)</p>
                          <p className="text-[9px] md:text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Oversight standards for health professionals' credentials and practice compliance.</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 md:space-y-3 pt-1 md:pt-2">
                    <div className="flex items-start gap-2 md:gap-3 p-2.5 md:p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-lg md:rounded-xl">
                      <CheckSquare className="w-3.5 h-3.5 md:w-4 md:h-4 text-emerald-600 dark:text-emerald-400 mt-0.5 md:mt-1 flex-shrink-0" />
                      <div>
                        <strong className="text-[10px] md:text-xs text-slate-900 dark:text-white uppercase">Guideline 5.1.1(a)(i) - Account Creation Disclosures</strong>
                        <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs mt-0.5">
                    ...
                    <div className="flex items-start gap-2 md:gap-3 p-2.5 md:p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-lg md:rounded-xl">
                      <CheckSquare className="w-3.5 h-3.5 md:w-4 md:h-4 text-emerald-600 dark:text-emerald-400 mt-0.5 md:mt-1 flex-shrink-0" />
                      <div>
                        <strong className="text-[10px] md:text-xs text-slate-900 dark:text-white uppercase">Guideline 5.1.1(b)(ii) - Explicit Account Deletion Button</strong>
                        <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs mt-0.5">
                    ...
                    <div className="flex items-start gap-2 md:gap-3 p-2.5 md:p-3 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-lg md:rounded-xl">
                      <CheckSquare className="w-3.5 h-3.5 md:w-4 md:h-4 text-emerald-600 dark:text-emerald-400 mt-0.5 md:mt-1 flex-shrink-0" />
                      <div>
                        <strong className="text-[10px] md:text-xs text-slate-900 dark:text-white uppercase">Guideline 5.1.1(b)(vii) - Medical Professional Verification Safety</strong>
                        <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs mt-0.5">
                  ...
                  {/* Kenya-Specific Data Protection Rights */}
                  <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-800 p-3 md:p-5 rounded-lg md:rounded-xl mt-3 md:mt-4">
                    <h3 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5 mb-2 md:mb-3">
                      <AlertTriangle className="w-3.5 h-3.5 md:w-4 md:h-4 text-emerald-600 dark:text-emerald-400" />
                      <span>Your Rights Under Kenya Data Protection Act, 2019</span>
                    </h3>
                    <ul className="space-y-1.5 md:space-y-2 text-[10px] md:text-xs text-slate-600 dark:text-slate-400">
                      <li className="flex items-start gap-1.5 md:gap-2"><CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-600 dark:text-emerald-400 mt-0.5 flex-shrink-0" /> <strong>Right to be Informed:</strong> You have the right to know how your data is collected and processed.</li>
                      <li className="flex items-start gap-1.5 md:gap-2"><CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-600 dark:text-emerald-400 mt-0.5 flex-shrink-0" /> <strong>Right to Access:</strong> Request a copy of all personal data we hold about you.</li>
                      <li className="flex items-start gap-1.5 md:gap-2"><CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-600 dark:text-emerald-400 mt-0.5 flex-shrink-0" /> <strong>Right to Rectification:</strong> Correct inaccurate or incomplete data in your profile.</li>
                      <li className="flex items-start gap-1.5 md:gap-2"><CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-600 dark:text-emerald-400 mt-0.5 flex-shrink-0" /> <strong>Right to Erasure (Right to be Forgotten):</strong> Request deletion of your account and all associated data.</li>
                      <li className="flex items-start gap-1.5 md:gap-2"><CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-600 dark:text-emerald-400 mt-0.5 flex-shrink-0" /> <strong>Right to Object:</strong> Opt out of specific data processing activities.</li>
                      <li className="flex items-start gap-1.5 md:gap-2"><CheckCircle className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-600 dark:text-emerald-400 mt-0.5 flex-shrink-0" /> <strong>Right to Data Portability:</strong> Receive your data in a structured, commonly used format.</li>
                    </ul>
                  </div>
                  ...
                  <div className="bg-slate-100 dark:bg-slate-800 p-2.5 md:p-3 rounded-lg md:rounded-xl font-mono text-[10px] md:text-xs text-slate-600 dark:text-slate-300 space-y-0.5 md:space-y-1">
                    <p> Email: medraenursing@gmail.com</p>
                    <p> Office: Nairobi, Kenya</p>
                    <p> Phone: 0704473503</p>
                    <p className="pt-1.5 md:pt-2 text-slate-400 dark:text-slate-500 text-[9px] md:text-[10px] border-t border-slate-200 dark:border-slate-700 mt-1.5 md:mt-2">For DPA-related complaints, you may also contact the Office of the Data Protection Commissioner (ODPC) at complaints@odpc.go.ke</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="hidden print:block pt-6 md:pt-8 border-t border-slate-200 dark:border-slate-800 text-center text-[10px] md:text-xs text-slate-400 dark:text-slate-500 mt-8 md:mt-12">
